...
async def start(client:Client, message): 
    ...
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        ...
        if not verify_id_info or verify_id_info["verified"]:
            await message.reply("<i>ʏᴏᴜ ᴀʀᴇ ᴀʟʀᴇᴀᴅʏ ᴠᴇʀɪꜰɪᴇᴅ ᴄʟɪᴄᴋ ᴏɴ ✅<b>ɢᴇᴛ ꜰɪʟᴇ</b>✅ ʙᴜᴛᴛᴏɴ ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ.....</i>")
            return  
...
async def donation(bot, message):
    ...
    yt=await message.reply_photo(photo='https://envs.sh/Tsr.jpg', caption=script.DONATE_TXT, reply_markup=InlineKeyboardMarkup(btn))
    await asyncio.sleep(300)
    await yt.delete()
